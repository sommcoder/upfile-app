...
/global;
dropzoneFileInput: HTMLInputElement | null;
null;
dropzoneHelpText: HTMLElement | null;
null;
dropzoneText: HTMLElement | null;
null;
dropzoneSelectBtn: HTMLElement | null;
null;
dropzoneFileSizeTally: HTMLElement | null;
null;
dropzoneFileSizeMax: HTMLElement | null;
null;
fileViewerList: HTMLElement | null;
null;
fileViewerOriginalRow: HTMLElement | null;
null;
fileViewerTrashIcon: HTMLElement | null;
null;
fileViewerStatus: HTMLElement | null;
null;
loadingSpinner: HTMLElement | null;
null;
fileViewerPlaceholder: HTMLElement | null;
null;
fileViewerErrorList: HTMLElement | null;
null;
fileViewerErrorItem: HTMLElement | null;
null;
fileNameSet: (Set) = new Set();
fileViewerUIMap: (Map) = new Map();
fileStateObj: (Record) = {};
errorMessages: string[] = [];
totalStateFileSize: number = 0;
formData: FormData = new FormData();
VALID_FILE_TYPES_OBJ: (Record) = {};
...
validateSubmittedFile(file, File);
boolean;
{
    this.errorMessages = [];
...
validateDraggedFile(file, DataTransferItem);
boolean;
{
    return Object.hasOwn(this.VALID_FILE_TYPES_OBJ, file.type);
}
formatToByteStr(byteSize, number);
string;
{
    let size = byteSize;
...
Promise < void  > {
    try: {
        ...
        : .errorMessages.length > 0 || validFilesArr.length === 0
    }
};
{
    this.renderErrorMessages(this.errorMessages);
...
handleFileAdd(value, { id: string }, status, FileStatus);
void {
    this: .renderFileViewerItem(this.fileStateObj[value.id]),
    : .updateFileStatus(value.id, status)
};
{
    this.addVariantProps(value.id);
    this.updateTallyElementText();
}
handleFileDelete(id, string);
void {
    this: .deleteVariantProps(id),
...
void {
    ...
    : .dropzoneSelectBtn || !this.dropzoneText, return: ,
